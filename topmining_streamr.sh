...
run_node(){
        ...
        do
                ...
                echo "配置文件目录：输入回车即可"
                echo "================================"
                docker run -it --rm -v $(cd ~/.streamrDocker${i}; pwd):/root/.streamr --name topmining_streamr_${i} topmininglabs/streamr-image:latest bin/config-wizard
                sleep 2
                p1=$(($port + j * 3))
                p2=$(($port + j * 3 + 1))
                p3=$(($port + j * 3 + 2))
                docker run -itd --restart always -p ${p1}:7170 -p ${p2}:7171 -p ${p3}:1883 -v $(cd ~/.streamrDocker${i}; pwd):/root/.streamr --name topmining_streamr_${i} topmininglabs/streamr-image:latest
...
check_logs_all() {
        ...
        if [ ${node_count1} = "1" ]; then
            ...
            all
                ...
                for ((i=$node_start; i<=$node_start + node_count2 - 2; i ++))
                do
                echo "-----------------------------------------------------"
                        str1="开始查询节点topmining_streamr_"
                        str2=${i}
